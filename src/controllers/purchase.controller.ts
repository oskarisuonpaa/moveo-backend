...
import { addPurchase, getAllPurchases, getLatestPurchaseByEmail, getPurchasesByEmail } from "../services/shop/purchases.service";
 ...
 */
export const addPurchaseController: RequestHandler<object, unknown, PurchaseData> = async (req, res) => {
  try {
...
export const getAllPurchasesController: RequestHandler = async (req, res) => {
  try {
    ...
    res.status(500).send((error as Error).message || "Error fetching purchases.");
  }
};

export const getPurchasesByEmailController: RequestHandler<{ email: string }> = async (req, res) => {
  ...
  try {
    ...
    res.status(500).send((error as Error).message || "Error fetching purchases by email.");
  }
}

export const getLatestPurchaseByEmailController: RequestHandler = async (req, res) => {
  const { email } = req.params;
  try {
    ...
    res.status(500).send((error as Error).message || "Error fetching latest purchase by email.");
  }
};